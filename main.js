...
function buildShinpoBoardItems(count) {
    ...
    for (i = 0; i < NOTE_ARTICLES.length; i++) {
    ...
    var items = [];
    var limit = Math.min(maxCount, source.length);
...
function refreshShinpoBoard() {
    if (!DESTINATIONS || !DESTINATIONS.shinpo_board) return;

    DESTINATIONS.shinpo_board.description = "掲示板には、その日ごとに違う読みものが何枚か貼られている。";
    DESTINATIONS.shinpo_board.flavor = "紙の端が、風で少しだけ揺れている。";
    DESTINATIONS.shinpo_board.menuTitle = "今日はどの記事を読みますか?";
